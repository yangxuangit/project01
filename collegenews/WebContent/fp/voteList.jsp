...
<div class="">
				...
				<tr>
					...
					<td>
						...
						<button class="btn btn-info btn-sm btnEdit" data-current-daily="${daily}" data-toggle="modal" data-target="#dailyEdit">
							  <a href="votenews?method=fpqueryVotecount&sid=${d.sid}">参与投票</a>
						</button>
					</td>
		...
		<ul class="pagination">
			<li><a href="votenews?method=fpquerynews&cp=${pu.prev}"><span>&laquo;</span></a></li>
			<c:forEach begin="${pu.start}" end="${pu.end}" var="i">
				<c:choose>
					...
					<c:otherwise>
						<li><a href="votenews?method=fpquerynews&cp=${i}">${i}</a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<li><a href="votenews?method=fpquerynews&cp=${pu.next}"><span>&raquo;</span></a></li>
		</ul>
