...
<body>
     <div class="row nav-justified">
		...
		<ul class="pagination" >
			<li><a href="news?method=fpfindAllMynews4&cid=4&cp=${pu.prev}"><span>&laquo;</span></a></li>
			<c:forEach begin="${pu.start}" end="${pu.end}" var="i">
				<c:choose>
					...
					<c:otherwise>
						<li><a href="news?method=fpfindAllMynews4&cid=4&cp=${i}">${i}</a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<li><a href="news?method=fpfindAllMynews4&cid=4&cp=${pu.next}"><span>&raquo;</span></a></li>
		</ul>													
			</div> 
		</div>
	</div>
